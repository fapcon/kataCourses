...
func (d *DAO) BuildSelect(tableName string, condition Condition, fields ...string) (string, []interface{}, error) {
	...
	cond := sq.And{}
	for field, value := range condition.Equal {
		cond = append(cond, sq.Eq{field: value})
	}
	for field, value := range condition.NotEqual {
		cond = append(cond, sq.NotEq{field: value})
	}
	if len(cond) > 0 {
		queryRaw = queryRaw.Where(cond)
	}

	for _, order := range condition.Order {
		if order.Asc {
			queryRaw = queryRaw.OrderBy(order.Field + " ASC")
		} else {
			queryRaw = queryRaw.OrderBy(order.Field + " DESC")
		}
	}
